...
export default function DashboardShell({ children, displayName = "John Snow" }: Props) {
        ...
        <div className="mx-auto max-w-7xl flex items-center justify-between px-6 py-4">
          ...
          <div className="relative">
            ...
            {openProfile && (
              ...
              >
                <div className="px-3 py-2 text-sm text-gray-500">Hi, {displayName.split(" ")[0]}!</div>
                <MenuItem href="#" label="My profile" emoji="🙋‍♂️" />
                ...
                <button
                  ...
                  // v4 (que é o seu caso): callbackUrl
      ...
      <div className="mx-auto max-w-7xl grid grid-cols-1 md:grid-cols-[260px_1fr] gap-6 px-4 md:px-6 py-6">
        <aside className="hidden md:block">
          <div className="rounded-2xl bg-primary text-white p-6">
            <div className="h-16 w-16 rounded-full bg-white/20 grid place-items-center text-xl mb-3">🖼️</div>
            <div className="font-semibold">{displayName}</div>
            <div className="text-sm opacity-80">Level 1</div>
            <span className="inline-block mt-3 text-xs bg-white/15 rounded-full px-2 py-1">Free Tier</span>
          </div>

          <ul className="mt-6 grid gap-2">
            {nav.map((n) => (
              <li key={n.href}>
                <Link className="flex items-center gap-3 rounded-xl px-3 py-2 hover:bg-gray-100" href={n.href}>
...
function MenuItem({ href, label, emoji }: { href: string; label: string; emoji: string }) {
  return (
    <Link
      href={href}
